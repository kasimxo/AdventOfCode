...
function checkCompactArea(robotsPosition) {
    let check = false
    for (let i = 0; i < robotsPosition.length; i++) {
        let curr = robotsPosition[i]
        if (robotsPosition.some((rob) => rob.x === curr.x - 1 && rob.y === curr.y)
            && robotsPosition.some((rob) => rob.x === curr.x + 1 && rob.y === curr.y)
            && robotsPosition.some((rob) => rob.x === curr.x && rob.y === curr.y + 1)
            && robotsPosition.some((rob) => rob.x === curr.x && rob.y === curr.y - 1)
            && robotsPosition.some((rob) => rob.x === curr.x - 1 && rob.y === curr.y - 1)
            && robotsPosition.some((rob) => rob.x === curr.x + 1 && rob.y === curr.y - 1)) {
            check = true
        }
    }
