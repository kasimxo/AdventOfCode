...
texto.forEach((linea) => {
    if (validarHabitacion(linea)) {
        //num += sumarId(linea)
        //mostrar el id
        let id = identificarId(linea)
        //mostrar el nombre de la hab
        let nombre = calcularNombre(linea, id)
...
function calcularNombre(linea, id) {
    linea = linea.replaceAll('-', '')
    let abc = 'abcdefghijklmnopqrstuvwxyzabcdefghijklmnopqrstuvwxyz'
    let cadena = ''
    for (let i = 0; i < linea.length; i++) {
        if (abc.indexOf(linea[i]) >= 0) {
            cadena += abc[abc.indexOf(linea[i]) + id % 26]
        }
    }
    return cadena
}